"""Analytic FLOP / BOP / memory profiler for BHViT.

Given a config.json (as in configs/<name>/config.json), walks the BHViT
architecture analytically — no PyTorch model is instantiated — and reports:
...
Counting matches the BHViT paper (Tab. 2):
  ...
  - only conv/linear MACs are counted; auxiliary ops (LayerNorm, softmax,
    ...
    and printed for reference
  - weight memory only — position embeddings, RPReLU/LayerScale/move
    parameters etc. are tracked separately as "other FP params"

Use --extended to see the auxiliary ops and "other params" rolled in.

Usage:
    python scripts/profile_model.py configs/BHViT_tiny/config.json
    python scripts/profile_model.py configs/BHViT_tiny/config.json --some-fp
    python scripts/profile_model.py configs/BHViT_tiny/config.json --extended
"""
...
@dataclass
class Counts:
    # Paper-style: MAC counts only from Conv2d / Linear weight-multiply work.
    fp_macs: int = 0
    bin_macs: int = 0
    ...
    # per-line breakdown for --extended
...
def profile(cfg, counts):
    ...
    # ---- 4 stages with 3 patch merges between them ----
    for stage_idx in range(4):
        ...
        for b in range(n_blocks):
            ...
            if not is_attn:
                # ---- MSGDC token mixer: 3× grouped 3x3 conv, groups=4 ----
