...
def timm_deit_to_hf_vit(timm_sd, num_layers=12):
    """Remap timm `deit_tiny_patch16_224` keys to HuggingFace ViTForImageClassification keys.

    Drops the head (incompatible num_classes); the HF model keeps its freshly
    initialized 10-class classifier.
    """
    new = {}
...
def main():
    ...
    p.add_argument("--config", default="configs/deit-tiny-patch16-224-cifar",
    ...
    remapped = timm_deit_to_hf_vit(timm_model.state_dict(), num_layers=cfg.num_hidden_layers)
    ...
    for epoch in range(args.epochs):
        ...
        print(f"epoch {epoch:3d} | loss {train_loss:.4f} | val acc {acc:.4f} | "
