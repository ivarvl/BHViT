"""Profile every config in the width-precision sweep and emit a CSV.

Writes one row per (config, precision-setting) covering:
  - the 5 binary variants:  d ∈ {64, 80, 96, 112, 128}, w/a = 1/1, --some-fp
  - the FP32 baseline:      d = 64, w/a = 32/32

The CSV is written to logs/sweep_profile.csv and also printed to stdout.

Numbers use the paper convention (Conv/Linear MACs only, BOPs/64). Pass
`--extended` for the "include aux FLOPs and other FP params" variant.

Usage:
    python scripts/profile_sweep.py
    python scripts/profile_sweep.py --num-classes 37
    python scripts/profile_sweep.py --extended
"""
...
RUNS = [
    ...
    ("bin-d80",   80,  1,  1, True),
    ("bin-d96",   96,  1,  1, True),
    ("bin-d112",  112, 1,  1, True),
    ("bin-d128",  128, 1,  1, True),
    ("bin-d144",  144, 1,  1, True),
    ("bin-d160",  160, 1,  1, True),
    ("bin-d176",  176, 1,  1, True),
    ("bin-d192",  192, 1,  1, True),
    ("bin-d208",  208, 1,  1, True),
    ("bin-d224",  224, 1,  1, True),
    ("bin-d240",  240, 1,  1, True),
    ("bin-d256",  256, 1,  1, True),
    ("fp32-d64",  64,  32, 32, False),
]


def run_one(d, wbits, abits, some_fp, num_classes, extended, bop_div):
    ...
    fp_bytes = c.fp_weight_params * 4
    other_bytes = c.other_fp_params * 4 if extended else 0
...
def main():
    ...
    for label, d, wbits, abits, some_fp in RUNS:
        r = run_one(d, wbits, abits, some_fp,
                    args.num_classes, args.extended, args.bop_divisor)
        r["label"] = label
        rows.append(r)

    with open(args.out, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["label"] + [k for k in rows[0] if k != "label"])
        w.writeheader()
        for r in rows:
            w.writerow(r)

    mode = "EXTENDED" if args.extended else "PAPER"
    print(f"# Mode: {mode}   BOP divisor: 1/{args.bop_divisor}   classes: {args.num_classes}")
    print(f"# CSV: {args.out}")
    print()
    print(f"{'label':<10} {'d':>4} {'w/a':>5}  "
          f"{'FP MACs':>9} {'Bin MACs':>10} {'OPs':>10}  "
          f"{'BinMem':>8} {'FPMem':>8} {'Total':>8}")
    print("-" * 84)
    for r in rows:
        print(f"{r['label']:<10} {r['d']:>4} "
